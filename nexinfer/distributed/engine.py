...
class DistributedEngine:
    ...
    def generate_stream(self, req: GenerationRequest) -> Generator[TokenOutput, None, None]:
        ...
        while True:
            ...
            logits = self.rank0.backend.prefill(req.request_id, input_ids if n == 0 else np.array([[tok_id]], dtype=np.int32))
            ...
            for stop in req.stop_sequences:
                if stop and stop in text:
                    finish = "stop"
            yield TokenOutput(text=text, token_id=tok_id, finish_reason=finish,
